...
if (!o.instance_name) {
  plv8.elog(INFO, `FATAL: instance_name is missing.`)
  throw 'fatal@28'
}
...
if (!package_md) {
  if (o.verbose >0) plv8.elog(INFO, `FATAL: package_key {{o.package_key}} not found.`)
...
const pfolder_id = plv8.execute("select content_folder__new($1,$2,$3,$4,$5)",
  [
  'publishers',         // 1: name
  'Publishers '+o.instance_name,                    // 2: label
...
const afolder_id = plv8.execute("select content_folder__new($1,$2,$3,$4,$5)",
  [
  'authors',         // 1: name
  'Authors '+o.instance_name,                    // 2: label
